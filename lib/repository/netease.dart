...
//      var result = json.decode(response.toString());
      ...
      String response =
          '{"data":[{"info":"","id":2116,"name":"陈奕迅","trans":null,"alias":["Eason Chan"],"albumSize":104,"mvSize":160,"picId":19165587183900212,"picUrl":"https://p2.music.126.net/_d1scu7z_1dmd0Zgv9mTLA==/19165587183900212.jpg","img1v1Url":"https://p1.music.126.net/_d1scu7z_1dmd0Zgv9mTLA==/19165587183900212.jpg"}],"hasMore":false,"count":1,"code":200}';
...
//      接口需要用postman调一次后才能用,暂用假数据
      String response =
          '{"code":200,"data":[{"type":1,"title":"汪苏泷为TFboys所创作的歌曲，可惜遗憾被退回，自己却唱火了！","durationms":184800,"creator":[{"userId":375931615,"userName":"流行音悦台"}],"playTime":2498168,"coverUrl":"http://p1.music.126.net/x__mmCZi8zh76Qs6d5DVkw==/109951163573581233.jpg","vid":"B8887F3E7341057ADAEFE22E01851DD4","aliaName":null,"transName":null,"alg":null,"markTypes":[]}],"hasMore":false,"count":1}';
      var result = json.decode(response);
...
//      var result = json.decode(response.toString());
  ...
  // 获取歌曲/专辑/歌单评论
  static getComment(int id, String type,
      {int offset = 0, int limit = 20}) async {
    var url = '${API_HOST}comment/$type?id=$id&offset=$offset&limit=$limit';
    try {
      Response response = await Dio().get(url);
      var result = json.decode(response.toString());
      return result;
